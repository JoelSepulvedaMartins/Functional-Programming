...
loop :: Int -> Int
loop n = loop' n 0
  where loop' 0 a = a -- caso base
        loop' n a = loop' (n - 1) (a + 1) -- caso recursivo

f :: Int->Int
f n
  | (n == 0) = 1  -- caso de parada
  | otherwise = 2 * f (n-1) -- caso de recursão

{-
  funções de ordem superior, funções que recebem como argumento, outras funções
